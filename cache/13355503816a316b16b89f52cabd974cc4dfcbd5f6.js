...
$('#todays-post, #carousel, #carousel li').css('width',$(window).width()+'px').css('height',$(window).height()+10+'px');$.localScroll();var top=$('#navigation').offset().top;$(window).scroll(function(event){var y=$(this).scrollTop();if(y>=top){$('#navigation').addClass('fixed');$('#filter').css('margin-top','58px');$('#scroll-down').fadeOut();}
else{$('#navigation').removeClass('fixed');$('#filter').css('margin-top','15px');$('#scroll-down').fadeIn();}});$('.back').hover(function(){$(this).stop(true,false).animate({opacity:1});},function(){$(this).stop(true,false).animate({opacity:0});});$('.carousel').cycle({fx:'scrollHorz',speed:'slow',timeout:0,rev:1,nowrap:1,next:'.previous',prev:'.next'});$('#related-carousel, #post-carousel li, #contributor-carousel').cycle({fx:'fade',speed:'fast'});$('.expand').click(function(){$("#tags").slideToggle();});$('#categories').infinitescroll({navSelector:".navigation",nextSelector:"a.next",itemSelector:"article",loadingImg:"/images/loader.gif",bufferPx:100});});$(window).resize(function(){$('#todays-post, #carousel, #carousel li').css('width',$(window).width()+'px').css('height',$(window).height()+'px');});