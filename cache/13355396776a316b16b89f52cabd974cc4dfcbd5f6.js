...
else{$('#navigation').removeClass('fixed');$('#filter').css('margin-top','15px');$('#scroll-down').fadeIn();}});$('.back').hover(function(){$(this).stop(true,false).animate({opacity:1});},function(){$(this).stop(true,false).animate({opacity:0});});$('.carousel').cycle({fx:'scrollHorz',speed:'slow',timeout:0,rev:1,nowrap:1,next:'.previous',prev:'.next'});$('#related-carousel li').cycle({fx:'fade',speed:'fast',timeout:0,pager:'#relatedcarousel-navigation'});$('#contributor-carousel li').cycle({fx:'fade',speed:'fast',timeout:0,pager:'#contributorcarousel-navigation'});$('.expand').click(function(){$("#tags").slideToggle();});$('#categories').infinitescroll({navSelector:".navigation",nextSelector:"a.next",itemSelector:"article",loadingImg:"/images/loader.gif",bufferPx:100});});$(window).resize(function(){$('#todays-post, #carousel, #carousel li').css('width',$(window).width()+'px').css('height',$(window).height()+'px');});