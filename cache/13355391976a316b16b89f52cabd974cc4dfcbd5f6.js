...
$('#todays-post, #carousel, #carousel li').css('width',$(window).width()+'px').css('height',$(window).height()+'px');$('#scroll-down').css('top',$("#branding").css("top")+10+'px');$.localScroll();var top=$('#navigation').offset().top;$(window).scroll(function(event){var y=$(this).scrollTop();if(y>=top){$('#navigation').addClass('fixed');$('#filter').css('margin-top','58px');$('#scroll-down').fadeOut();}
