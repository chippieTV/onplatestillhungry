...
;(function($){
  ...
  $.fn.infinitescroll = function(options,callback){
    ...
    function determinePath(path) {
		...
		// there is a 9 in the url surrounded by slashes, e.g. /page/9/
		if (path.match(/^(.*?)\bP9\b(.*?$)/)) {

			//console.log('good times');
			path = path.match(/^(.*?)\bP9\b(.*?$)/)[1] + "P";

		} else 
        // if there is any 9 in the url at all.
        if (path.match(/^(.*?)9(.*?$)/)) {

			// page= is used in django:
			//   http://www.infinite-scroll.com/changelog/comment-page-1/#comment-127
			if (path.match(/^(.*?page=)9(\/.*|$)/)) {
				path = path.match(/^(.*?page=)9(\/.*|$)/).slice(1);
				return path;
			}

			//console.log('Trying backup next selector parse technique. Treacherous waters here, matey.');
			path = path.match(/^(.*?)9(.*?$)/).slice(1);
    ...
    function kickOffAjax(){
		...
		//console.log(path + props.currPage);
    ...
    function loadCallback(){
		...
		var total = $('#paginationtotal').text() * 9;

		if (props.currPage >= total) {
  ...
  $.infinitescroll = {     
        defaults: {
			...
			loadingText     : "Hold on we're just fetching some more cool stuff",
			donetext        : "Sorry we seem to have run out of cool stuff.",
